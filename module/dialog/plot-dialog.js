export class PlotDialog extends Dialog {
    ...
    async _ready(event) {
      ...
      let chatData = {"content": (this.ready) ? game.i18n.localize("MAGICALOGIA.ReadyPlot") : game.i18n.localize("MAGICALOGIA.AwaitPlot"), "speaker": ChatMessage.getSpeaker({ alias: this.name })};
      ChatMessage.create(chatData);
    
      if (game.user.id === this.receiver) {
        let plot = game.magicalogia.plot.find(a => (a.actorId === this.actorId));
        ...
        game.socket.emit("system.magicalogia", {id: "resp", sender: game.user.id, receiver: this.receiver, data: { actorId: this.actorId, dice: this.plot, ready: this.ready } });
