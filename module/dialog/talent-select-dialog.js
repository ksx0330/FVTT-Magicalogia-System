import { Dialog, mergeObject } from "../compat.js";

export class TalentSelectDialog extends Dialog {

  constructor(actor, callback, options) {
    super({
      title: "Select Talent",
      content: "",
      buttons: {}
    }, options);

    this.actor = actor;
    this.select = null;
    this.data.content = this.getContent();
    this.data.buttons = {
      confirm: {
        icon: '<i class="fas fa-check"></i>',
        label: game.i18n.localize("MAGICALOGIA.Confirm"),
        callback: () => {
          const name = $(this.select).text().split("/")[0];
          callback(name);
        }
      },
      Tmp: {
        icon: '<i class="fas fa-dice"></i>',
        label: game.i18n.localize("MAGICALOGIA.Tmp"),
        callback: () => callback(game.i18n.localize("MAGICALOGIA.Tmp"))
      }
    };
    this.data.default = "confirm";
  }

  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      classes: ["magicalogia", "dialog"],
      width: 600
    });
  }

  activateListeners(html) {
    super.activateListeners(html);
    html.find(".select").click(this._selectDice.bind(this));
  }

  getContent() {
    const table = Array.from({ length: 12 }, () => []);
    for (let j = 0; j < 6; ++j) {
      const name = String.fromCharCode(65 + j);
      table[0].push({
        title: `${game.i18n.localize(`MAGICALOGIA.${name}1`)} ${game.i18n.localize("MAGICALOGIA.Tmp")}`,
        num: 0
      });
    }

    for (let i = 2; i <= 12; ++i) {
      for (let j = 0; j < 6; ++j) {
        const name = String.fromCharCode(65 + j);
        table[i - 1].push({
          title: game.i18n.localize(`MAGICALOGIA.${name}${i}`),
          num: this.actor ? this.actor.system.talent.table[j][i - 2].num : 0
        });
      }
    }

    let content = `<table><tr>`;
    for (const block of table[0]) content += `<th class="select">${block.title}</th>`;
    content += `</tr>`;

    for (let i = 1; i < 12; ++i) {
      content += `<tr>`;
      for (const block of table[i]) {
        let text = block.title;
        if (block.num != 0) text += `/${block.num}`;
        content += `<td class="select">${text}</td>`;
      }
      content += `</tr>`;
    }
    content += `</table>`;
    return content;
  }

  _selectDice(event) {
    event.preventDefault();
    if ($(event.currentTarget).hasClass("talent-select")) {
      $(event.currentTarget).removeClass("talent-select");
      this.select = null;
      return;
    }
    $(event.currentTarget).parent().parent().find(".talent-select").removeClass("talent-select");
    $(event.currentTarget).addClass("talent-select");
    this.select = event.currentTarget;
  }

}
